...
import util_file #change by xiaojun
...
try:
    ...
    def run(args, stdin=None, stdout=None, stderr=None, shell=False,
        ...
        # cannot use PIPE.
        if stdout_pipe:
            stdout_fn = util_file.mkstempfname('.stdout')  ##change util.file to util_file by xiaojun
            stdout = open(stdout_fn, 'wb')
        if stderr_pipe:
            stderr_fn = util_file.mkstempfname('.stderr') ##change util.file to util_file by xiaojun
...
def available_cpu_count():
    ...
    try:
        def get_cpu_val(name):
            return float(util_file.slurp_file('/sys/fs/cgroup/cpu/cpu.'+name).strip())  ##change util.file to util_file by xiaojun
