...
class CondaPackage(InstallMethod):
    ...
    def execute(self, cmd, loglevel=logging.DEBUG, buffered=None, check=None, silent=None, stderr=None):
        ...
        if cmd[0] in self.QUIET_COMMANDS:
            run_cmd.extend(['-q', '-y'])
        run_cmd.extend(cmd)
        result = util_misc.run_and_print(  #change util.misc to util_misc by xiaojun
    ...
    def __init__(
        ...
        if "CONDA_PREFIX" in os.environ and len(os.environ["CONDA_PREFIX"]):
            ...
            if conda_env_path:
                if os.path.isdir(conda_env_path):
                    ...
                    result = util_misc.run_and_print(["conda", "env", "list", "--json"], silent=True, env=os.environ)  #change util.misc to util_misc by xiaojun
                    if result.returncode == 0:
                        command_output = result.stdout.decode("UTF-8")
        ...
        if env_root_path or env or not self.env_path:
            env_root_path = env_root_path or os.path.join(util_file.get_project_path(), 'tools', 'conda-tools') #change util.file to util_file by xiaojun
            env = env or 'default'
        ...
        conda_cache_path = conda_cache_path or os.path.join(util_file.get_project_path(), 'tools', 'conda-cache') #change util.file to util_file by xiaojun
    ...
    def _patch(self, path, patch):
        ...
        patch_path = os.path.join(
            util_file.get_project_path(), 'tools', 'patches', patch)  #change util.file to util_file by xiaojun
    ...
    def verify_install(self):
        ...
        if self.installed:
            ...
            if (not self.post_verify_cmd_executed) and self.post_verify_command:
                ...
                result = util_misc.run_and_print(post_verify_command, silent=False, check=False, env=self.conda_env, cwd=self.bin_path) #change util.misc yo util_misc by xiaojun
...
class DownloadPackage(InstallMethod):
    ...
    def __init__(
        ...
        if destination_dir is None:
            destination_dir = util_file.get_build_path()  #change util.file to util_file by xiaojun
        self.url = url
    ...
    def download(self):
        ...
        util_file.mkdir_p(download_dir)  #change util.file to util_file by xiaojun
    ...
    def unpack(self, download_dir):
        ...
        util_file.mkdir_p(self.destination_dir) #change util.file to util_file by xiaojun
