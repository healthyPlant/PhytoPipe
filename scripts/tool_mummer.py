...
class MummerTool(tools.Tool):
    ...
    def trim_contigs(self, refFasta, contigsFasta, outFasta,
        ...
        if aligner=='nucmer':
            ...
            raise NameError()
        delta_1 = util_file.mkstempfname('.delta') #change util.file tp util_file by xiaojun
        delta_2 = util_file.mkstempfname('.delta')
        tiling = util_file.mkstempfname('.tiling')
    ...
    def scaffold_contigs(self, refFasta, contigsFasta, outFasta,
        ...
        if aligner=='nucmer':
            ...
            raise NameError()
        delta_1 = util_file.mkstempfname('.delta') #change util.file to util_file by xiaojun
        delta_2 = util_file.mkstempfname('.delta')
        tiling = util_file.mkstempfname('.tiling')
        aligner(refFasta, contigsFasta, delta_1, extend=extend, breaklen=breaklen,
    ...
    def scaffold_contigs_custom(self, refFasta, contigsFasta, outFasta,
        ...
        fs = util_misc.FeatureSorter()  ##change util.misc to util_misc by xiaojun
        with util_file.open_or_gzopen(tiling, 'rU') as inf:  #change util.file to util_file by xiaojun
            for line in inf:
                row = line.rstrip('\n\r').split('\t')
                c = row[11]
        ...
        alnReaders = {}
        aln_file = util_file.mkstempfname('.aligns') #change util.file to util_file by xiaojun
        ...
        with open(outFasta, 'wt') as outf:
            for c in [seqObj.id for seqObj in Bio.SeqIO.parse(refFasta, 'fasta')]:
                ...
                def frac_unambig(seqs):
                    ...
                    util_misc.chk(n_diff_lens(alt_seqs_f) == 1, 'ambig_max_lens>1 not currently supported')  #change util.misc to util_misc by xiaojun
                    n_tot = len(seqs[0])
                ...
                for line in util_file.fastaMaker([(str(c)+"_contigs_ordered_and_oriented", ''.join(seq))]):  #change util.file to util_file by xiaojun
                    outf.write(line)
        ...
        if outAlternateContigs and alternate_contigs:
            ...
            with open(outAlternateContigs, 'wt') as outf:
                for c, left, right, seqs in alternate_contigs:
                    for line in util_file.fastaMaker([  #change util.file to util_file by xiaojun
    ...
    def align_one_to_one(self, refFasta, otherFasta, outFasta):
        ...
        util_file.makeFastaFile(seqs, outFasta) #change util.file to util_file by xiaojun
...
def contig_chooser(alt_seqs, ref_len, coords_debug=""):
    ...
    if not alt_seqs:
        ...
        ranks = list(sorted(util_misc.histogram(alt_seqs).items(),  #by xiaojun
        ...
        if len(ranks)==1:
            ...
            len_ranks = list(sorted(util_misc.histogram(   #by xiaojun
