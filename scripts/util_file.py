...
#import util.cmd #comment by xiaojun
import util_misc
...
def check_paths(read=(), write=(), read_and_write=()):
    ...
    '''
    read, write, read_and_write = map(util_misc.make_seq, (read, write, read_and_write))  #change util.misc to util_misc by xiaojun
...
def set_tmp_dir(name):
    ...
    for e in ('LSB_JOBID', 'LSB_JOBINDEX', 'JOB_ID'):
        if e in os.environ:
            ...
            break
    tempfile.tempdir = tempfile.mkdtemp(prefix='-'.join(proposed_prefix) + '-', dir='/tmp') #dir=util.cmd.find_tmp_dir()), changed by xiaojun
...
def extract_tarball(tarfile, out_dir=None, threads=None, compression='auto', pipe_hint=None):
    ...
    if out_dir is None:
        ...
        util_file.mkdir_p(out_dir)  #change util.file to util_file by xaiojun
    ...
    if compression == 'zip':
        ...
        if compression == 'gz':
            decompressor = ['pigz', '-dc', '-p', str(util_misc.sanitize_thread_count(threads))]  #change util.misc to util_misc by xiaojun
        elif compression == 'bz2':
            decompressor = ['lbzip2', '-dc', '-n', str(util_misc.sanitize_thread_count(threads))] #change util.misc to util_misc by xiaojun
...
def concat(inputFilePaths, outputFilePath, append=False):
    ...
    with open(outputFilePath, 'a' if append else 'w') as outfile:
        for filePath in util_misc.make_seq(inputFilePaths):  #change util.misc to util_misc by xiaojun
...
def grep_count(file_path, to_match, additional_flags=None, fixed_mode=True, starts_with=False):
    ...
    number_of_seqs = util_misc.run_and_print(cmd, silent=False, check=True, env=env)  #change util.misc to util_misc by xiaojun
...
def repack_tarballs(out_compressed_tarball,
                    ...
                    pipe_hint_out=None,
                    threads=None):
    threads = util_misc.sanitize_thread_count(threads)  #change util.misc to util_misc by xiaojun
