	...
	shell:
		...
		if (( $fileNum > 0 )); then
			...
			do
				...
				#in a re-run, sometimes the old reference is not in the selected file (.selectedRef.txt), check whehter the reference ($refName) is available, if yes, run compareSeq.py
